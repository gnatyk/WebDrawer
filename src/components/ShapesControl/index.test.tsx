...
describe('<ShapesControl />', () => {
  ...
  it('Should call bucketFillHandler with Bucket Fill input values', () => {
    ...
    expect(validateBusketFill).toHaveBeenCalledWith(point1, color.fillBusketColor, canvasLayout);

    expect(validateBusketFill).toBeCalledTimes(1);
    expect(bucketFillHandler).toHaveBeenCalledWith(point1, color.fillBusketColor);
  });
  ...
  const submitShapeForm = (points: any, type: string, wrapper: any) => {
    if (type === 'busket_fill') {
      ...
      wrapper
        .find(Input)
        .at(0)
        .simulate('change', { target: { value: points.point1.x + 1 } }); //x1
      wrapper
        .find(Input)
        .at(1)
        .simulate('change', { target: { value: points.point1.y + 1 } }); //y1
      wrapper.find('input').simulate('change', { target: { value: color.fillBusketColor } }); //color
    } else {
      expect(wrapper.find(Input)).toHaveLength(4);
      wrapper
        .find(Input)
      ...
      wrapper
        .find(Input)
        .at(1)
        .simulate('change', { target: { value: points.point2.x + 1 } }); //x2
      wrapper
        .find(Input)
        .at(3)
        .simulate('change', { target: { value: points.point2.y + 1 } }); //y2
    }
